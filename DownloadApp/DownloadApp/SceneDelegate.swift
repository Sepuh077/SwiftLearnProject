...
class SceneDelegate: UIResponder, UIWindowSceneDelegate {
    ...
    func scene(_ scene: UIScene, willConnectTo session: UISceneSession, options connectionOptions: UIScene.ConnectionOptions) {
        ...
        // open when typing url in browser
        let storyboard = UIStoryboard(name: "Main", bundle: nil)
        
        guard let rootVC = storyboard.instantiateViewController(identifier: "1") as? ViewController else {
    ...
    func scene(_ scene: UIScene, openURLContexts URLContexts: Set<UIOpenURLContext>) {
        if let url = URLContexts.first?.url {
            ...
            if component.count > 1, let appId = component.last {
                ...
                currentVC?.notificationLabel.text = appId
                currentVC?.notification.isHidden = false
            }
        }
    }
